...
func set_pointlist( point_list) -> void:
	pointlist = point_list

func set_size_box(left_wall: int, right_wall: int, floor: int, ceiling: int) -> void:
	size = [left_wall, right_wall, floor, ceiling]

# Sizebox is in [left wall, right wall, floor, ceiling]
func generate( point_list : Array) -> void:
	pointlist = point_list
...
func relax() -> void:
	...
	generate(new_pnt_lst)	# Only the points are changing
